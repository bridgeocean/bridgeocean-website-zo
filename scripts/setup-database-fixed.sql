...
CREATE POLICY "Admins can view all bookings" ON charter_bookings FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE id::text = auth.uid()::text AND role = 'admin'));
CREATE POLICY "Admins can manage bookings" ON charter_bookings FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE id::text = auth.uid()::text AND role = 'admin'));

-- Fleet vehicles - public read, admin write
CREATE POLICY "Anyone can view fleet" ON fleet_vehicles FOR SELECT USING (true);
CREATE POLICY "Admins can manage fleet" ON fleet_vehicles FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE id::text = auth.uid()::text AND role = 'admin'));
...
CREATE POLICY "Admins can view partner registrations" ON partner_registrations FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE id::text = auth.uid()::text AND role = 'admin'));
CREATE POLICY "Admins can manage partner registrations" ON partner_registrations FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE id::text = auth.uid()::text AND role = 'admin'));

-- Driver candidates - admins only
CREATE POLICY "Admins can manage driver candidates" ON driver_candidates FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE id::text = auth.uid()::text AND role = 'admin'));
